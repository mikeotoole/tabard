...
###
  ...
  def permissions_for_resource(resource)
    ...
    return permission_match ? permission_match : Permission.new(role: self, subject_class: resource.class, id_of_subject: resource.id, _destroy: true)
  end
  def nested_permissions_for_resource(resource)
    case resource.class.to_s
      when "DiscussionSpace"
        ...
        return permission_match ? permission_match : Permission.new(role: self, subject_class: "Discussion", parent_association_for_subject: "discussion_space", id_of_parent: resource.id, _destroy: true)
      else
        return nil
    end
  end
