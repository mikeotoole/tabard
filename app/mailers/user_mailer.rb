...
class UserMailer < ActionMailer::Base
  default :from => "noreply@crumblin.com" # TODO Joe, Set to real email value. -MO
  ...
  def setup_admin(user, password)
    @resource = user
    mail(:to => @resource.email, :subject => 'New Crumblin Admin Created', :tag => 'password-reset', :content_type => "text/html") do |format|
