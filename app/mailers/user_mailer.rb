...
class UserMailer < ActionMailer::Base
  ...
  def password_reset(user, password=nil)
    @resource = user
    mail(to: @resource.email, subject: 'Guild.io - Password reset notification', tag: 'password-reset') do |format|
  ...
  def setup_admin(user, password=nil)
    @resource = user
    mail(to: @resource.email, subject: 'Guild.io - New admin account', tag: 'password-reset') do |format|
  ...
  def password_changed(user_id)
    ...
    mail(to: @resource.email, subject: 'Guild.io - Password has been changed') do |format|
