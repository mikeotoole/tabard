...
describe "ActiveAdmin Wow" do
  ...
  let(:swtor) { DefaultObjects.swtor }
  ...
  describe "#index" do 
    it "returns 200 when logged in as superadmin" do
      ...
      current_url.should == admin_swtors_url
    end 
  ...
  describe "#show" do 
    it "returns 200 when logged in as superadmin" do
      ...
      page.status_code.should == 200
      current_url.should == admin_swtor_url(:id => swtor.id)
    end 
  ...
  describe "#new" do 
    it "returns 200 when logged in as superadmin" do
      ...
      page.status_code.should == 200
      current_url.should == new_admin_swtor_url
    end 
  ...
  describe "#edit" do 
    it "returns 200 when logged in as superadmin" do
      ...
      page.status_code.should == 200
      current_url.should == edit_admin_swtor_url(:id => swtor.id)
    end 
  ...
  describe "#create" do
    it "creates Swtor when logged in as superadmin" do
      login_as superadmin
      expect {
        page.driver.post("/admin/swtors", { :swtor => attributes_for(:swtor, :server => "test_server") } )
      }.to change(Swtor, :count).by(1)
    end 
    
    it "creates Swtor when logged in as admin" do
      login_as admin
      expect {
        page.driver.post("/admin/swtors", { :swtor => attributes_for(:swtor, :server => "test_server") } )
      }.to change(Swtor, :count).by(1)
  ...
  describe "#update" do 
    it "updates Swtor when logged in as superadmin" do
      login_as superadmin
      
      page.driver.put("/admin/swtors/#{swtor.id}", { :swtor => { :server_name => "test_case_server" } } )
      Swtor.find(swtor).server_name.should eql "test_case_server"
    end
    
    it "updates Swtor when logged in as admin" do
      login_as admin
    ...
    it "returns 403 when logged in as moderator" do
      ...
      orginal_server = swtor.server_name
      page.driver.put("/admin/swtors/#{swtor.id}", { :swtor => { :server_name => "test_case_server" } } )
  ...
  describe "#destroy" do 
    it "raises error ActionNotFound" do
      lambda { page.driver.delete("/admin/swtors/#{swtor.id}") }.should raise_error(AbstractController::ActionNotFound)
    end  
  end
end
